...
const ScrapingInfoViewer = ({ scrapingInfo }) => {
  ...
  if (!scrapingInfo)
    return <p className="no-analysis">No scraping data available.</p>;
  ...
  const renderStructured = () => {
    ...
    return (
      <div className="scraping-section">
        <p>
          <strong>Headings:</strong> {headings.join(", ") || "None"}
        </p>
        <p>
          <strong>Links:</strong> {links.length}
        </p>
        <p>
          <strong>Forms:</strong> {forms.length}
        </p>
        ...
        {redFlags.suspiciousInlineEvents?.length > 0 && (
          <div>
            <p>
              <strong>Suspicious Inline Events:</strong>
            </p>
            <ul>
  ...
  const renderActiveTab = () => {
    switch (activeTab) {
      ...
      case "crawled":
        return renderCrawledLinks();
      case "raw":
        return renderRawHtml();
      case "screenshot":
  ...
  return (
    <div className="scraping-info-viewer">
      <h2>
        <FaListAlt /> Scraping & Crawling Data
      </h2>
