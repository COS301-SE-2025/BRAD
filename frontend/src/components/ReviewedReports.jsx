...
const ReviewedReports = ({ reports, onSelect }) => {
  const reviewed = useMemo(
    () => reports.filter(r => r.investigatorDecision),
    [reports]
  );
  ...
  const getScores = (r) => {
    ...
    const combined = parts.reduce((a, b) => a + b, 0); // 0–200
    const avg = combined / parts.length;               // 0–100
    ...
    <div>
      ...
      <div className="report-grid">
        ...
        {reviewed.map(report => {
          ...
          return (
            <div
              className={`report-card-grid reviewed ${isHighCard ? 'high-risk' : ''}`}
              key={report._id}
            >
