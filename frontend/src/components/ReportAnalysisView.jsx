...
export default function ReportAnalysisView({
  ...
  loggedInUser = null,
  onRefresh = () => {},
  onClose = () => {}, // optional: called when user wants to close page view
}) {
  ...
  const confirmAction = async () => {
    ...
    if (pendingVerdict === "claim") {
      ...
      try {
        ...
        // onClose(); // page probably stays open
      } catch (err) {
        console.error(err);
      ...
      try {
        ...
        router.push("/investigator/in-progress");      } catch (err) {
        console.error(err);
  ...
  const EvidenceGrid = () => (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-3">
      {evidence.length === 0 && <div className="text-sm text-gray-500">No evidence</div>}
      {evidence.map((file, i) => {
        ...
        return (
          <div key={i} className="border rounded p-2">
            {isImage ? (
              ...
              <a href={`/api/static/uploads/evidence/${file}`} download className="text-blue-600 underline">
                {file}
              </a>
            )}
          </div>
  ...
  return (
    <div>
      ...
      <ConfirmationModal
        ...
        message={
          pendingVerdict === "claim" ? "Are you sure you want to claim this report?" : `Are you sure you want to mark this report as ${pendingVerdict}?`
        }
        confirmText="Yes"
        cancelText="Cancel"
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {/* Back button navigates to in-progress page */}
        <button
          onClick={() => router.push("/investigator/in-progress")}
      ...
      {/* Investigator/Admin views (same as modal) */}
      <div className="space-y-6">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-4 mt-2">
            ...
            <InfoCard icon={<FaCalendarAlt />} label="SSL Expiry" value={analysis.sslExpires ? new Date(analysis.sslExpires).toLocaleString() : ""} />
            ...
            <InfoCard icon={<FaCalendarAlt />} label="Analysis Timestamp" value={analysis.timestamp ? new Date(analysis.timestamp).toLocaleString() : ""} />
          </div>
        </section>
        ...
        {role === "investigator" && view === "in-progress" && !report.investigatorDecision && (
          <div className="flex gap-3">
            <button className="px-4 py-2 bg-red-500 text-white rounded flex items-center gap-2" onClick={() => handleDecision("malicious")} disabled={isDeciding}>
              <MdDangerous />
              {isDeciding ? "Submitting..." : "Mark Malicious"}
            </button>
            <button className="px-4 py-2 bg-green-500 text-white rounded flex items-center gap-2" onClick={() => handleDecision("benign")} disabled={isDeciding}>
              <FaCheckCircle />
...
function InfoCard({ label, value, icon = null }) {
  if (!value && value !== 0) return null; // show 0 values
  return (
