...
export default function ReportAnalysisModal({
  ...
  const siteRiskReasons = useMemo(() => {
    ...
    const allRedirects = uniq(
      scrapingInfo.pages.flatMap((p) => p.flags?.redirectChain || [])
    );
    const allSuspiciousJS = uniq(
      scrapingInfo.pages.flatMap((p) => p.flags?.suspiciousJS || [])
    );
    const flaggedCount = scrapingInfo.pages.filter(
      (p) => (p.riskScore ?? 0) > 0
    ).length;
    const malwarePages = scrapingInfo.pages.filter(
      (p) => p.flags?.malwareDetected
    ).length;
    const obfusPages = scrapingInfo.pages.filter(
      (p) => p.flags?.obfuscatedScripts
    ).length;
    const httpOnHttps = scrapingInfo.pages.reduce(
      (a, p) => a + (p.flags?.httpOnHttpsCount || 0),
      0
    );
    const errorResponses = scrapingInfo.pages.reduce(
      (a, p) => a + (p.flags?.errorResponses || 0),
      0
    );
    const kwMatches = scrapingInfo.pages.reduce(
    ...
    if (malwarePages)
      reasons.push(`Malware indicators on ${malwarePages} pages`);
    ...
    if (allSuspiciousJS.length)
      reasons.push(`${allSuspiciousJS.length} suspicious JS snippets`);
    if (allRedirects.length > 3)
      reasons.push(`Long redirect chains observed (${allRedirects.length})`);
    return reasons;
  }, [scrapingInfo]);
  ...
  const confirmAction = async () => {
    ...
    if (pendingVerdict === "claim") {
      ...
      setIsDeciding(true);
      try {
        await API.patch(`/report/${report._id}/decision`, {
        ...
        setNotification({
          type: "success",
          title: "Success",
          message: `Report marked as ${pendingVerdict}`,
        });
        setOpen(false);
      } catch (err) {
        console.error(err);
          ...
          <div className="modal-overlay items-start justify-center py-10 px-4">
            ...
            >
              ...
              <ConfirmationModal
                ...
                title={
                  pendingVerdict === "claim"
                    ? "Confirm Claim"
                    : "Confirm Decision"
                }
                message={
                  pendingVerdict === "claim"
                    ? "Are you sure you want to claim this report?"
                    : `Are you sure you want to mark this report as ${pendingVerdict}?`
                }
                confirmText="Yes"
                cancelText="Cancel"
              ...
              {role === "general" && (
                <div className="space-y-4">
                  ...
                  <InfoCard
                    icon={<FaCheckCircle />}
                    label="Verdict"
                    value={report.investigatorDecision ?? "N/A"}
                  />
                  <InfoCard
                    icon={<FaShieldAlt />}
                    label="Status"
                    value={getDisplayStatus(report)}
                  />

                  <section>
                    <ScreenshotGallery
                      screenshots={
                        Array.isArray(report.scrapingInfo?.screenshots)
                          ? report.scrapingInfo.screenshots.map((p) => {
                              ...
                              return p.startsWith("http")
                                ? p
                                : `/api/static/screenshots/${cleanPath}`;
                            })
                          : []
                      }
                    />
                  ...
                  {view !== "pending" && (
                    <div className="space-y-6">
                      ...
                      <InfoCard
                        icon={<FaUser />}
                        label="Investigator"
                        value={report.reviewedBy?.username}
                      />
                      <InfoCard
                        icon={<FaChartLine />}
                      ...
                      {s.endTime && (
                        <InfoCard
                          icon={<FaClock />}
                          label="Scan End"
                          value={new Date(s.endTime).toLocaleString()}
                        />
                      )}
                      {s.durationMs && (
                        <InfoCard
                      ...
                      <section>
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-2">
                          ...
                          <InfoCard
                            icon={<FaMapMarkerAlt />}
                            label="Country"
                            value={analysis.geo?.country}
                          />
                          <InfoCard
                            icon={<FaBuilding />}
                            label="ASN/Org"
                            value={analysis.geo?.asn}
                          />
                          <InfoCard
                            icon={<FaCalendarAlt />}
                            label="Domain Created"
                            value={analysis.stats?.domain_created}
                          />
                          <InfoCard
                            icon={<FaClock />}
                            label="Domain Age(days)"
                            value={analysis.stats?.domain_age_days}
                          />
                          <InfoCard
                            icon={<FaClock />}
                            label="SSL Days Remaining"
                            value={analysis.stats?.ssl_days_remaining}
                          />
                          <InfoCard
                            icon={<FaClock />}
                            label="NS Count"
                            value={analysis.stats?.dns?.ns_count}
                          />
                          <InfoCard
                            icon={<FaClock />}
                            label="MX Count"
                            value={analysis.stats?.dns?.mx_count}
                          />
                          <InfoCard
                            icon={<FaCalendarAlt />}
                      ...
                      <section>
                        ...
                        <div className="p-3 bg-gray-100 dark:bg-gray-800 rounded mt-2">
                          <div>
                            <b>Level:</b> {analysis.riskLevel}
                          </div>
                          <div>
                            <b>Score:</b> {analysis.riskScore}
                          </div>
