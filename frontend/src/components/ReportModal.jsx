...
const ReportModal = ({
  ...
  const pages = useMemo(() => {
    if (Array.isArray(scrapingInfo.pages) && scrapingInfo.pages.length)
      return scrapingInfo.pages;
    return scrapingInfo &&
      (scrapingInfo.url || scrapingInfo.finalUrl || scrapingInfo.startUrl)
      ? [scrapingInfo]
      : [];
  }, [scrapingInfo]);
  ...
  const siteRiskReasons = useMemo(() => {
    ...
    const httpOnHttps = pages.reduce(
      (a, p) => a + (p.flags?.httpOnHttpsCount || 0),
      0
    );
    const errorResponses = pages.reduce(
      (a, p) => a + (p.flags?.errorResponses || 0),
      0
    );
    const kwMatches = pages.reduce(
      (a, p) => a + (p.flags?.keywordMatches || 0),
      0
    );
    ...
    if (flaggedCount)
      reasons.push(`${flaggedCount} page${sfx(flaggedCount)} flagged`);
    if (malwarePages)
      reasons.push(
        `Malware indicators on ${malwarePages} page${sfx(malwarePages)}`
      );
    if (obfusPages)
      reasons.push(
        `Obfuscated scripts on ${obfusPages} page${sfx(obfusPages)}`
      );
    if (kwMatches)
      reasons.push(`${kwMatches} suspicious keyword match${sfx(kwMatches)}`);
    if (httpOnHttps)
      reasons.push(
        `${httpOnHttps} HTTP→HTTPS downgrade/mixed-content attempt${sfx(
          httpOnHttps
        )}`
      );
    if (errorResponses)
      reasons.push(
        `${errorResponses} error response${sfx(errorResponses)} (4xx/5xx)`
      );
    if (susJsSnippets)
      reasons.push(
        `${susJsSnippets} suspicious JS snippet${sfx(susJsSnippets)}`
      );
    if (redirects > 3)
      reasons.push(`Long redirect chains observed (${redirects})`);
    return reasons;
      ...
      <div className="report-modal">
        ...
        <div className="heading-section">
          {[
            ...
            {
              label: "Investigator",
              value: report.reviewedBy?.username || "Unassigned",
            },
          ].map(({ label, value }, idx) => (
              ...
              <div className="forensic-block">
                ...
                <div className="summary-cards">
                  {[
                    ...
                    },
                    {
                      ...
                      label: "Registrar",
                      value: analysis.registrar,
                    },
                    {
                      icon: <User size={18} />,
                      label: "WHOIS Owner",
                      value: analysis.whoisOwner,
                    },
                    {
                      icon: <Lock size={18} />,
                      label: "SSL Valid",
                      value: analysis.sslValid ? "Yes" : "No",
                    },
                    {
                      icon: <Calendar size={18} />,
                      label: "SSL Expiry",
                      value: analysis.sslExpires,
                    },
                    {
                      icon: <RefreshCcw size={18} />,
                      label: "Reverse IP",
                      value: analysis.reverseIp,
                    },
                ...
                {analysis.geo && (
                  <div className="info-section">
                    <h4>
                      <Globe size={18} /> Hosting Info
                    </h4>
                    <p>
                      <strong>Country:</strong> {analysis.geo.country || "N/A"}
                    </p>
                    <p>
                      <strong>ASN / Org:</strong> {analysis.geo.asn || "N/A"}
                    </p>
                  </div>
                )}
                ...
                {analysis.stats && (
                  <div className="info-section">
                    ...
                    <div className="stats-grid">
                      <p>
                        <strong>Domain created:</strong>{" "}
                        {analysis.stats.domain_created || "N/A"}
                      </p>
                      <p>
                        <strong>Domain age:</strong>{" "}
                        {fmt(analysis.stats.domain_age_days)} days
                      </p>
                      <p>
                        <strong>SSL days remaining:</strong>{" "}
                        {fmt(analysis.stats.ssl_days_remaining)}
                      </p>
                      <p>
                        <strong>NS count:</strong>{" "}
                        {fmt(analysis.stats.dns?.ns_count)}
                      </p>
                      <p>
                        <strong>MX count:</strong>{" "}
                        {fmt(analysis.stats.dns?.mx_count)}
                      </p>
                      <p>
              ...
              <div className="forensic-block">
                ...
                <div className="risk-summary">
                  <p>
                    ...
                    <span
                      className={`risk-badge ${
                        report.analysis.riskLevel?.toLowerCase() || ""
                      }`}
                    >
                  ...
                  <p>
                    ...
                    {Number.isFinite(forensicScoreRaw)
                      ? Math.round(forensicScoreRaw)
                      : "N/A"}
                  </p>
                    ...
                    <div className="info-section">
                      ...
                      <ul className="risk-reasons">
                        {Array.isArray(analysis.riskReasons)
                          ...
                          : Object.entries(analysis.riskReasons).map(
                              ([k, v]) => (
                                <li key={k}>
                                  <strong>{labelize(k)}:</strong> {String(v)}
                                </li>
                              )
                            )}
                      </ul>
          ...
          <div className="report-column scraping-column">
            {showFullDetails && (
              <>
                ...
                {(s?.siteRiskLevel ||
                  Number.isFinite(siteScoreRaw) ||
                  siteRiskReasons.length > 0) && (
                  <div className="info-section">
                    <h3 className="section-title">
                      <Shield size={18} /> Scraping &amp; Crawling Data: Risk
                      Analysis
                    </h3>
                    ...
                    {showWhois && (
                      <div className="table-wrapper whois-table-wrapper">
                        <table className="styled-table">
                          <tbody>
                            {Object.entries(analysis.whoisRaw).map(
                              ([key, value]) => (
                                <tr key={key}>
                                  <td>
                                    <strong>{key}</strong>
                    ...
                    {showDns && (
                      <div className="table-wrapper dns-table-wrapper">
                        <table className="styled-table">
                          ...
                          <tbody>
                            {Object.entries(analysis.dns).flatMap(
                              ([type, entries]) =>
